...
class PengajuanSuratController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            return PengajuanSurat::dataTable($request);
        }
    ...
    {
        if ($request->ajax()) {
            return PengajuanSurat::dataTableArsip($request);
        }

        return view('arsipSurat.index', [
            'title' => 'Arsip Surat',
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            LogActivity::create([
                ...
                'action' => 'membuat pengajuan atas nama ' . $request->nama_lengkap,
            ]);

            PengajuanSurat::createPengajuanSurat($request);
            DB::commit();
    ...
    public function approve(Request $request, PengajuanSurat $pengajuanSurat)
    {
        ...
        try {
            ...
            LogActivity::create([
                'user_id' => session('userData')->id,
                'action' => 'approve Pengajuan ' . $pengajuanSurat->nama_lengkap,
            ]);

            // Ambil template view
            $template = 'pdf.TemplateSurat.' . $pengajuanSurat->suratType->template_view;

            // Generate nomor surat
            $nomorSurat = PengajuanSurat::generateNomorSurat();
            ...
            // Generate PDF via Service (Return String)
            $pdfContent = PdfService::create(
                html: $html,
                filename: $fileName,
                format: "A4",
                orientation: "P",
                outputMode: "S" // penting! biar return string pdf
            );

            // Simpan PDF ke storage
            Storage::put('public/surat/' . $fileName, $pdfContent);
            ...
            return \Res::update("Pengajuan Berhasil di Setujui");
        } catch (\Exception $e) {
            DB::rollback();
    ...
    public function downloadSurat($id)
    {
        ...
        return FileDownloadService::downloadPdf(
            'surat/' . $surat->file_surat,
    ...
    public function reject(Request $request, PengajuanSurat $pengajuanSurat)
    {
        ...
        try {
            ...
            LogActivity::create([
                'user_id' => session('userData')->id,
                'action' => 'reject Pengajuan ' . $pengajuanSurat->nama_lengkap,
            ]);
            ...
            return \Res::update("Pengajuan di Tolak");
        } catch (\Exception $e) {
            DB::rollback();
            return \Res::error($e->getMessage());
        }
